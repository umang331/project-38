...
function preload(){
  ...
  pterosaursi = loadImage("assets/dino.png");
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyWentDown("space") && trex.y  >= height-120) {
      jumpSound.play()
      trex.velocityY = -10;
    }
...
function spawnObstacles() {
  if(frameCount % 400 === 0){
    var pterosaurs  = createSprite(600,650,20,20);
    pterosaurs.addImage(pterosaursi)
    pterosaurs.setCollider('circle',0,0,45)
    pterosaurs.velocityX = -(6 + 3*score/100);
    pterosaurs.scale = 0.1;
    pterosaurs.lifetime = 300;
    pterosaurs.depth = trex.depth;
    trex.depth +=1;
    obstaclesGroup.add(pterosaurs);
  }
  if(frameCount % 80 === 0) {
